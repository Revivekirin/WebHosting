...
def json_rich_header(file_path):
    pe = pefile.PE(file_path)
    try:
        rich_header = pe.parse_rich_header()
        if rich_header is not None:
            rich = {
                "Key" : rich_header.keys
            }
            ...
            if 'records' in rich_header:
                ...
                for record in records:
                    ...
                    rich["Records"].append(rich_record)
...
def all_pe_info(file_path):
    try:
        pe_info = {
            ...
            "Rich Header": json_rich_header(file_path) or {},
            "Hash": json_hash(file_path) or {},
            # "contains_another_exe": find_executables(file_path) or {},
...
def save_json_result(file_path):
    try:
        ...
        ascii_file_path = os.path.join(os.path.dirname(file_path), ascii_file_name)
        unicode_file_path = os.path.join(os.path.dirname(file_path), unicode_file_name)
...
FILELIST_directory = "/home/ubuntu/WHS/TM/FILELIST"
latest_file_path = find_latest_file(FILELIST_directory)
...
base_directory = "/home/ubuntu/WHS/TM/collection"
automate_anlyze()